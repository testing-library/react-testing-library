...
<summary>How do I update attributes of rendered element and trigger events?</summary>

The `getBy*` helpers give you access to the DOM elements that is rendered by React.
In your tests you are free to modify any attribute on these DOM element.
To trigger the event listeners in React that you want to test you can use the [`Simulate`](#simulate) utility.
...
// First we modify the values of our input elements
usernameNode.value = fakeUser.username
passwordNode.value = fakeUser.password

// Then we trigger the submit event on the form
Simulate.submit(formNode)

// Now we can assert that everything worked
expect(handleSubmit).toHaveBeenCalledTimes(1)
expect(handleSubmit).toHaveBeenCalledWith(fakeUser)
```

[Open the tests](https://github.com/kentcdodds/react-testing-library/blob/master/src/__tests__/forms.js)
for a full example of this.
